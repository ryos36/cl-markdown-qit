...
;----------------------------------------------------------------
; tagged-list をみて (:comment . "zzz" ) (nl) (:comment . "yyy") (nl) や
; (:string-single-quote . "zzz" ) (nl) (:string-single-quote . "yyy")
; が続いていたら統合して (:comment "zzz" "yyy") にする
; このとき :nl は消費するのであとで復活させないといけない
;
; #'python-tagged-list-to-who-style も参照されたい
;
  ...
  (labels ((is-target-keyword (k)
    ...
    (if (null lst) (nreverse rv)
      (let ((word (car lst))
        ...
        (if (atom word) (concat-tagged-list remain (push word rv))
          (let ((key (car word)))
